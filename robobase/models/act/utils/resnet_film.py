# from: https://github.com/robopen/roboagent/blob/main/detr/models/resnet_film.py
...
class ResNetWithExtraModules(nn.Module):
    ...
    def __init__(
        ...
        in_channels_conv1 = (
            ...
            if (
                ...
                and film_config.get("append_object_mask", None) is not None
            )
            else 3
        )
        ...
        for layer_idx, layer in enumerate(
            ...
            for block_idx, block in enumerate(layer):
                if film_feat_per_block[block_idx] is not None:
                    assert (
                        x.shape[0] == film_feat_per_block[block_idx].shape[0]
    ...
    def forward(
        self, x: Tensor, film_features: List[Optional[Tensor]], **kwargs
    ) -> Tensor:
    ...
    if weights is not None:
        ...
        if float(torch.__version__.split(".")[1]) < 7:
            ...
            state_dict = torch.hub.load_state_dict_from_url(
                model_urls[kwargs.get("arch")], progress=progress
            )
